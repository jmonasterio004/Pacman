...
var GF = function(){
 ...
 // variables para contar frames/s, usadas por measureFPS
    ...
    var Ghost = function(id, ctx){
		...
		this.lastRow = 0;
		this.lastCol = 0;
		this.sprites = [new Sprite('./res/img/sprites.png', [456, 16*this.id + 65],[16,16], 0.005, [0,1]),
						new Sprite('./res/img/sprites.png', [456+16*8,65],[16,16], 0.005, [0,1]),
						new Sprite('./res/img/sprites.png', [456+16*10,65],[16,16], 0.005, [0,1]),
						new Sprite('./res/img/sprites.png', [456+16*8,65+16],[16,16], 0.005, [0,1])];
	...
	this.draw = function(){
		...
		this.ctx.save();
		this.ctx.translate(this.x-radio,this.y-radio);
		if(this.state == Ghost.NORMAL){			
			this.sprites[0].render(this.ctx);			
		}else if(this.state == Ghost.VULNERABLE){
			if(thisGame.ghostTimer > 100){				
				this.sprites[1].render(this.ctx);				
			}else{					
				var v = thisGame.ghostTimer;
				if(v > 80){
					this.sprites[1].render(this.ctx);
				}else if(v > 60){
					this.sprites[2].render(this.ctx);
				}else if(v > 40){
					this.sprites[1].render(this.ctx);
				}else if(v > 20){
					this.sprites[2].render(this.ctx);
			...
			// Pintar ojos
			this.sprites[3].render(this.ctx);	
		}	
		this.ctx.restore();
	...
	Pacman.prototype.move = function(delta) {
		player.sprite.update(delta);
		for (var i=0; i< numGhosts; i++){
			for(var j=0; j< 4; j++){
				ghosts[i].sprites[j].update(delta);
			}			
		}
		// Tu código aquí
     ...
     Pacman.prototype.draw = function(x, y) {		
		...
		var pY = player.y;
		ctx.save();	
		ctx.translate(pX-radio,pY-radio);		
    ...
    var mainLoop = function(time){
		...
		//Comprobamos si estamos en pausa
		if(thisGame.mode == thisGame.PAUSA){
		}else{
	...
	reset();
	//Cargamos las imagenes de sprites
	resources.load([
		'./res/img/sprites.png'
	]);
